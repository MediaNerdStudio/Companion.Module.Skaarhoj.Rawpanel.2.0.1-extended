exports.UpdateActions = function (self) {
	const buttonLED = [
		{ id: '0', label: 'OFF' },
		{ id: '5', label: 'Dimmed' },
	...
	self.setActionDefinitions({
		SetButtonState: {
			name: 'Set Button LED Status On/Off/Dimmed',
			options: [
				hwc,
				{
					type: 'dropdown',
			...
			for (let index = 0; index < this.sdData.keys.length; index++) {
				...
				if (keyData.color !== '') {					
					...
					if (rgb == 128 + 64) {
						...
						this.sendCommand('HWCc#' + color_key + '=' + rgb)
...
exports.sendCommand = async function (message) {
	if (message !== undefined) {
		// this.log('debug', 'sending', message, 'to', this.config.host)
